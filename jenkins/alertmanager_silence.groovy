...
pipeline {
  ...
  post {
    ...
    failure {
      slackSend channel: "#jenkins",
        color: "danger",
        message: "@here FAILED alert silenced:\nJob: <${env.BUILD_URL}|${env.JOB_BASE_NAME} ${env.BUILD_DISPLAY_NAME}>\nLabel: ${env.SILENCE_LABEL}\nValue: ${env.SILENCE_VALUE}\nDuration: ${env.SILENCE_DURATION}"
    }
          ...
          script {
            sh(
              ...
              for COUNT in {1..10}; do
                ...
                if [[ ${APOD} = alertmanager-* ]]; then
                  break
                else
                  echo "Sleeping for 3 seconds. Count: ${COUNT} of 10"
                  sleep 3
                fi
              done
              ...
              FORWARDER=$!
              ansible-playbook alertmanager-silence.yml -e alertmanager_silence_label_name=${SILENCE_LABEL} -e alertmanager_silence_label_value=${SILENCE_VALUE} -e alertmanager_silence_duration=${SILENCE_DURATION} -e alertmanager_url=localhost:${PORT}
              kill ${FORWARDER}
            ''',
            )
          }
