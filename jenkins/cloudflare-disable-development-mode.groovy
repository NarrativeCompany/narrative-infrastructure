def sendSuccess() {
  if (env.SLACK_NOTIFY == 'yes') {
    slackSend channel: "#jenkins",
      color: "good",
      message: ":mostly_sunny: SUCCESS Disabled Cloudflare development mode\nJob: <${env.BUILD_URL}|${env.JOB_BASE_NAME} ${env.BUILD_DISPLAY_NAME}>\nZone: ${env.ZONE}"
  }
}
...
pipeline {
  ...
  post {
    failure {
      slackSend channel: "#jenkins",
        color: "danger",
        message: "@here FAILED Disabling Cloudflare development mode FAILED :red_circle:\nJob: <${env.BUILD_URL}|${env.JOB_BASE_NAME} ${env.BUILD_DISPLAY_NAME}>\nZone: ${env.ZONE}"
    }
        ...
        ]) 
        {
          script {
            sh(
              ...
              CRESULT=`curl -X PATCH "https://api.cloudflare.com/client/v4/zones/${ZONEID}/settings/development_mode" \
