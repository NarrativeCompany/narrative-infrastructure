...
kubectl -n it create configmap alertmanager-config --from-file=config/config.yml  --dry-run -o json | kubectl apply -f -

echo "Deleting rendered config file..."
rm config/config.yml

echo "Deploying alertmanager-template..."
kubectl -n it create configmap alertmanager-template --from-file=template/narrative.tmpl --dry-run -o json | kubectl apply -f -

echo "Pause 5 seconds to let configmaps update..."
sleep 5

if kubectl -n it get pods | grep alertmanager | grep Running; then
  echo "reloading config"
  kubectl -n it get pods | grep alertmanager | awk '{ print $1 }' | xargs -I {} kubectl -n it exec {} -- /bin/sh -c "kill -HUP 1"
fi
...
# kubectl apply -f alertmanager-narrative-cloud-ingress.yml
