resource "google_sql_database_instance" "staging-master" {
  name = "staging-master"
...
resource "google_sql_database_instance" "staging-failover" {
  name = "staging-failover"
  master_instance_name = "${ google_sql_database_instance.staging-master.name }"
  database_version = "${ var.db_version }"
  project = "${ var.project }"
  region = "${ var.region }"
...
resource "google_sql_database" "sql-database" {
  name = "sql-database"
  instance = "${ google_sql_database_instance.staging-master.name }"
  project = "${ var.project }"
}
